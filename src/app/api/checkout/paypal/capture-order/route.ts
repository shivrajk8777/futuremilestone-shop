        ...
        const itemsListHtml = items.map((item: any) => {
          ...
          return `
            <tr style="border-bottom: 1px solid #e2e8f0;">
              ...
              <td style="padding: 12px 8px; vertical-align: middle; font-size: 14px; color: #334155; text-align: right; white-space: nowrap;">${typeof item.price === "number" ? item.price.toLocaleString("en-IN") : item.price}</td>
              <td style="padding: 12px 8px; vertical-align: middle; font-size: 14px; color: #0f172a; text-align: right; font-weight: 700; white-space: nowrap;">${typeof lineTotal === "number" ? lineTotal.toLocaleString("en-IN") : lineTotal}</td>
            </tr>
          `;
        }).join('');
        ...
        const emailHtml = `
          ...
          <body style="font-family: 'DM Sans', Arial, sans-serif; background-color: #f8fafc; margin: 0; padding: 20px;">
            <div style="max-width: 600px; margin: 0 auto; background: #fff; padding: 30px; border-radius: 16px; border: 1px solid #ececec;">
              ...
              <p style="font-size: 14px; color: #334155; line-height: 1.6;">Hi ${user.name || 'Customer'}, thank you for your payment via PayPal! Your order has been placed successfully.</p>
              ...
              <h3 style="text-align: right; color: #0f172a; margin-top: 20px; font-size: 16px;">Total Paid: ${total}</h3>
