...
function normalizeStatus(statusInput: any, events: TrackingEvent[] = []): string {
  let text = "";
...
const PROVIDERS: Record<string, {
  ...
  track: (id: string, dispatchTime: Date) => Promise<TrackingDetails>;
}> = {
  dhl: {
    name: "DHL Express",
    trackingUrl: (id) => `https://www.dhl.com/en/express/tracking.html?AWB=${encodeURIComponent(id)}`,
    track: async (id, dispatchTime) => {
      ...
      const allCheckpoints: { delayHrs: number; location: string; description: string; status: string }[] = [
      ...
      const activeEvents: TrackingEvent[] = allCheckpoints
